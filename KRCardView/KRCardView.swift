...
public protocol KRCardView {
    ...
    var containerBlurEffect: UIBlurEffect? { get }
...
extension KRCardView where Self: UIViewController {
    ...
    public func addKRCardView() {
        ...
        cardViewController.visualEffectView.effect = self.containerBlurEffect
        cardViewController.visualEffectView.backgroundColor = self.containerBackgroundColour ?? UIColor.black.withAlphaComponent(0.5)
        cardViewController.visualEffectView.alpha = 0
    ...
    public func removeKRCardView() {
        cardViewController.visualEffectView.removeFromSuperview()
        cardViewController.view.removeFromSuperview()
        cardViewController.removeFromParent()
    }
    
    public func showKRCardView() {
        guard self.nextState == .expanded else {
            return
        }
    ...
    public func hideKRCardView() {
        guard self.nextState == .collapsed else {
            return
        }
...
fileprivate extension UIViewController {
    ...
    func animationTransitionIfNeeded(state: CardState, duration: TimeInterval) {
        ...
        var blurAnimator: UIViewPropertyAnimator {
            ...
            let animator = UIViewPropertyAnimator(duration: duration, dampingRatio: 1) {
                bottomView.cardViewController.visualEffectView.alpha = (state == .expanded) ? 1 : 0
            }
